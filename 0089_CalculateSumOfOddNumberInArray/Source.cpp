...
int CalculateSumOfOddNumbersInArray() {
	int sum = 0;
	for (int i = 0; i < SizeOfArray; i++) {
		if (!IsEven(Items[i])) {
			sum += Items[i];
...
void PrintSumOfOddNumbersInArray(int sum) {
	cout << "\nSumation of Odd numbers in the array is: " << sum << endl;
}
...
int main() {
	ReadItemsToArray();
	PrintItemsOfArray();
	PrintSumOfOddNumbersInArray(CalculateSumOfOddNumbersInArray());
	return 0;
}